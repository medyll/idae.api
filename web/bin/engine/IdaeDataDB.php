<?
	...
	class IdaeDataDB extends IdaeConnect {
		...
		/**
		 * @param MongoCollection|string $appscheme_name_or_instance_or_datascheme
		 */
		public function __construct($appscheme_name_or_instance_or_datascheme = null) {

			parent::__construct();

			try {
				if (empty($appscheme_name_or_instance_or_datascheme)) {
			...
			$argument                 = $appscheme_name_or_instance_or_datascheme;
			$this->AppDataScheme      = null;//new AppDataScheme($argument);
			$this->appscheme_instance = $this->get_collection_from_code($argument);
		 ...
		 */
		public function find($query_vars = [], $projection = []) {

			$rs                   = $this->appscheme_instance->find($query_vars, $projection);
			$this->cursor_results = $rs;

			$sort = $this->get_sort();
			if ($sort) $rs->sort($sort);

			$rs->skip(($this->nbRows * $this->page))->limit($this->nbRows);

			/*if ($this->page && $this->nbRows) {$rs->skip(($this->nbRows * $this->page))->limit($this->nbRows);}
			elseif ($this->page && $this->nbRows)*/
		...
		function update_id($table_value, $fields = [], $upsert = true) {
