<?
	...
	// skelMdl::send_cmd('act_notify',['msg'=>'Agent non connecté '.session_id('d4uuh6a4jca153fk9rphfl0l27'),'options'=> ['mdl'=>'app/app_login/app_login','sticky'=>1]],"d4uuh6a4jca153fk9rphfl0l27");
	...
	//
	if (droit('DEV') && $DEBUG) {
		skelMdl::send_cmd('act_notify', ['msg' => '__________________<br><pre>  POST  => ' . json_encode($_POST, JSON_PRETTY_PRINT) . '</pre>', 'options' => ['sticky' => 1, 'id' => 'json_debug']], session_id());
	}
	//
	if (droit('DEV') && $DEBUG) {
	...
	//
	if (droit('DEV') && $DEBUG) {
		skelMdl::send_cmd('act_notify', ['msg' => '<pre> DATA QUERY <br> ' . $count . ' : ' . vardump($vars + $where, true) . '/  ' . $sortBySecond . " => " . $sortDirSecond . ' / ' . $page . '</pre>', 'options' => ['sticky' => 1, 'id' => 'json_debug']], session_id());
	}
	// ACT COUNT
	...
	if (!empty($groupBy)):
		if (!empty($GRILLE_FK[$groupBy])) {
			...
			$rs_dist        = $APP->distinct_all($groupBy, array_merge($vars, $where));
	...
	if (!empty($groupBy)):
		...
		foreach ($rs_dist as $arr_dist):
			$i++;
			if (!empty($GRILLE_FK[$groupBy])) {
				$vars['id' . $groupBy] = (int)$arr_dist['id' . $groupBy]; // unset apres
			}
			$rs = $APP->query($vars + $where)->sort([$sortBy => $sortDir]);
			//
			...
			$data_main[] = ['html' => $z, 'table' => $groupBy, 'table_value' => $arr_dist['id' . $groupBy], 'vars' => $vars_rfk, 'groupBy' => 1, 'maxcount' => $dist_max_count, 'count' => $dist_count];

			$count_frag      = 0;
			$count_frag_test = 0;

			$count_groupBy     = $rs->count();
			$count_groupBy_max = $rs->count(true);
			...
			while ($arr = $rs->getNext()) {
				...
				$data_main[] = array_merge($data_row, ['chunk' => $count_frag_test, 'maxcount' => $dist_max_count, 'count' => $dist_count]);

				// progress
				if ($i == 1 || ($i % 100) == 0 || !$rs->hasNext()) {
					$progress_vars = ['progress_name' => 'progress_' . $table, 'progress_value' => ($nbRows * $page) + $i, 'progress_max' => $rs_dist->count()];
		...
		if ($rs_noGroupBy->count() != 0):
			...
			$data_main[] = ['html' => $z, 'vars' => $vars_rfk, 'groupBy' => 1];
			$strm[]      = ['html' => $z, 'table' => $groupBy, 'table_value' => '', 'vars' => $vars_rfk, 'groupBy' => 1];
	...
	if (empty($groupBy)):
		...
		while ($arr = $rs->getNext()) {
			...
			$data_main[] = array_merge($data_row, ['chunk' => $count_frag_test, 'maxcount' => $maxcount, 'count' => $count]);
	...
	function dotr($table, $arr) {
		...
		foreach ($GRILLE_COUNT as $key_count => $field):
			...
			$RS_TMP                     = $APP_TMP->find([$id => $arr[$id]], [$id => 1]);
			...
			$count_grille               = (empty($RS_TMP->count())) ? '' : $RS_TMP->count();
			$out['count_' . $key_count] = '<a onclick="' . $link . '"  $attr >' . $count_grille . '</a>' . ((!empty($RS_TMP->count())) ? "<span class='count_title'> $key_count</span>" : '');
		endforeach;
		...
		return array_merge(['html' => $out, 'vars' => $trvars], $out_more);
	}
